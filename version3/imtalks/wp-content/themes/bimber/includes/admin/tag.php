...
/**
 * Add custom fields on a post tag edit screen.
 *
 * @param object $tag      Current taxonomy term object.
 * @param string $taxonomy Current taxonomy slug.
 */
function bimber_post_tag_add_custom_fields( $tag, $taxonomy ) {
	// Header.
		...
		<tr class="form-field term-fe-template-wrap">
			...
			<td>
				...
				bimber_ui_render_image_radio( array(
					...
					'img_base_url'  => BIMBER_ADMIN_DIR_URI . 'images/templates/featured-entries/',
				) );
				?>
			</td>
		</tr>
		...
		<tr class="form-field term-template-wrap">
			...
			<td>
				...
				$templates = array_merge(
					array(
					'' => array(
							'label' => sprintf( __( 'default (%s)', 'bimber' ), $templates[ $global_template ] ),
							'path'  => BIMBER_ADMIN_DIR_URI . 'images/templates/archive/inherit.png',
						),
					),
					bimber_get_archive_templates()
				);
...
/**
 * Save custom fields on a post tag edit screen.
 *
 * @param int $term_id  Term ID.
 */
function bimber_post_tag_save_custom_fields( $term_id ) {
