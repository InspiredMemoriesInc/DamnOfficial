...
function adace_get_coupon( $coupon_id = '' ) {
	...
	if ( ! empty( $coupon_thumbnail ) ) {
		$output .= '<div class="coupon-thumbnail">' . $coupon_thumbnail . '</div>';
	}
	$output .= '<h3 class="coupon-title g1-gamma g1-gamma-1st">' . get_the_title( $coupon_id ) . '</h3>';
	if ( $coupon_discount ) {
		$output .= '<p class="coupon-discount">' . wp_kses_post( $coupon_discount ) . '</p>';
	}
	if ( $coupon_discount_code ) {
		$output .= '<div class="coupon-copy-wrap">';
		$output .= '<a class="coupon-copy" href="#"><span class="coupon-code">' . wp_kses_post( $coupon_discount_code ) . '</span><span class="coupon-action g1-button g1-button-solid g1-button-s" data-copied="' . esc_html( 'Copied', 'adace' ) . '">' . esc_html( 'Copy', 'adace' ) . '</span></a>';
		$output .= '</div>';
	}
	$output .= '</div>';
	$output .= '</div>';
