...
function adace_shoppable_image_front_enqueue_styles() {
	wp_enqueue_style( 'shoppable-images-css', trailingslashit( adace_get_plugin_url() ) . 'includes/shoppable-images/assets/css/shoppable-images-front.min.css' );
}
...
function adace_shoppable_image_shortcode( $atts ) {
	...
	ob_start();
	?>
	<div id="shoppable-image-<?php echo( esc_attr( $shoppable_image_id ) ); ?>" class="shoppable-image">
		<style scoped="scoped">
				...
				<div class="shoppable-image-pin shoppable-image-pin-<?php echo( esc_attr( $pin_key ) ); ?>">
					...
					<div class="shoppable-image-pin-body">
						...
						if ( 'custom_product' === $pin['type'] ) {
							if ( ! empty( $pin['name'] ) ) {
								echo( wp_kses_post( '<h5 class="g1-epsilon g1-epsilon-1st pin-name">' . $pin['name'] . '</h5>' ) );
							}
							if ( ! empty( $pin['price'] ) ) {
						...
						if ( 'woocommerce' === $pin['type'] && blogstar_can_use_plugin( 'woocommerce/woocommerce.php' ) ) {
							...
							if ( ! empty( $product_thumb ) ) {
								echo( wp_kses_post( '<p class="pin-thumb"><img width="120" height="120" src="' . esc_url( $product_thumb ) . '" /></p>' ) );
							}
							if ( ! empty( $pin_name ) ) {
								echo( wp_kses_post( '<h5 class="g1-epsilon g1-epsilon-1st pin-name">' . $pin_name . '</h5>' ) );
							}
							if ( ! empty( $pin_price ) ) {
...
add_action( 'amp_post_template_css', 'adace_shoppable_amp_load_css', 10 );
/**
 * Load AMP (Accelerated Mobile Pages) CSS
 */
function adace_shoppable_amp_load_css() {
	$css = '';
