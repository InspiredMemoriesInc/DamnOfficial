			...
			<table class="form-table form-wrap">
				<tbody>
					...
					<tr class="adace-metabox-tab embed <?php echo( esc_attr( 'embed' === $current_related_products_type ? 'current' : '' ) ); ?>">
						...
						<td>
							<textarea style="width:100%;" rows="6" name="adace_related_products_embed" id="adace_related_products_embed" ><?php echo( wp_kses_post( $current_related_products_embed ) ); ?></textarea>
...
function adace_save_post_related_products_metabox( $post_id ) {
	...
	if ( isset( $_POST['adace_related_products_embed'] ) ) {
		$input_sanitized['products_embed'] = filter_var( $_POST['adace_related_products_embed'], FILTER_SANITIZE_SPECIAL_CHARS );
	} else {
