...
class Adace_Get_My_Widget extends WP_widget {
	...
	public function form( $instance ) {
		...
		<p class="adace-widget-tab embed <?php echo( esc_attr( 'embed' === $instance['type'] ? 'current' : '' ) ); ?>">
			...
			<textarea
				...
				rows="6"
			><?php echo wp_kses_post( $instance['embed_code'] ); ?></textarea>
		</p>
	...
	public function widget( $args, $instance ) {
		...
		if ( $disclosure && ! empty( $disclosure_text ) ) {
			echo wp_kses_post( '<p class="g1-disclosure g1-meta">' . html_entity_decode( $disclosure_text ) . '</p>' );
		}
		if ( 'woocommerce' === $type ) {
		...
		if ( 'embed' === $type ) {
			echo wp_kses_post( '<div class="adace_get_my_widget_embed">' . $instance['embed_code'] . '</div>' );
		}
